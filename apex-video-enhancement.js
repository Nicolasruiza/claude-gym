...
function ensureStyles(){
  ...
  .workoutFormLink{margin:0 12px 5px!important;min-height:42px!important}
  .singleModeNote{margin:0 0 12px;padding:9px 11px;border-radius:10px;background:rgba(255,138,50,.10);border:1px solid rgba(255,138,50,.2);font-size:10px;color:#d8dde1}
  `;
...
function removeLogTab(){
  ...
  [...bottom.querySelectorAll('.nav')].forEach(b=>{if((b.querySelector('.navLabel')?.textContent||'').trim()==='Log')b.remove();});
...
function normalizeProgramLinks(){
  document.querySelectorAll('#programList .exercise').forEach(card=>{
    const name=(card.querySelector('h4')?.textContent||'').trim(),link=card.querySelector('.formLink');
    if(name&&link)link.href=yt(name);
  });
}
function decorateLift(lift){
  const name=(lift.dataset.name||lift.querySelector('h4')?.textContent||'').trim();if(!name)return;
  const e=meta(name);if(!e)return;
  const load=recommendedLoad(name);
  const top=lift.querySelector('.liftTop');
  if(top&&!top.querySelector('.workoutExercisePhoto')){
    top.classList.add('apexPhotoTop');
    const photo=document.createElement('img');photo.className='workoutExercisePhoto';photo.src=exerciseImage(name);photo.alt=name;photo.onerror=()=>safeImg(photo);top.prepend(photo);
  }
  if(!lift.querySelector('.liftPrescription')){
    const box=document.createElement('div');box.className='liftPrescription';
    const plate=plateBreakdown(name,load);
    box.innerHTML=`<div class="loadRow"><div><div class="loadLabel">Recommended load</div><strong>${name==='Pull Up'?'Bodyweight':load+' lb'}</strong></div><div class="loadLabel">${e[2]} sets · ${e[3]} reps</div></div>${plate?`<div class="plateLine">${plate}</div>`:''}`;
    top?.after(box);
  }
  ...
  if(!lift.querySelector('.workoutFormLink')){
    const link=document.createElement('a');link.className='formLink workoutFormLink';link.href=yt(name);link.target='_blank';link.rel='noopener';link.innerHTML=youtubeIcon+' Watch proper form';
    const sets=lift.querySelector('.sets');if(sets)lift.insertBefore(link,sets);else lift.appendChild(link);
  }
}
function decorateWorkout(){
  document.querySelectorAll('#workoutList .lift').forEach(decorateLift);
  if(singleExercise)applySingleMode();
}
function renderHomeFlow(){
  const home=document.querySelector('#home .content');if(!home)return;
  let mount=document.getElementById('apexHomeFlow');
  if(!mount){mount=document.createElement('div');mount.id='apexHomeFlow';mount.className='homeFlow';home.appendChild(mount);}
  const d=day();
  const exercises=d.groups.flatMap(g=>g[2]);
  const signature=activeDay+'|'+exercises.map(e=>e[0]+':'+recommendedLoad(e[0])).join('|');
  ...
  const cards=exercises.map(e=>{
    const load=recommendedLoad(e[0]);const plate=plateBreakdown(e[0],load);
    return `<button class="homeExercise" onclick="startApexExercise('${e[0].replace(/'/g,"\\'")}')"><img src="${exerciseImage(e[0])}" alt="${e[0]}" onerror="safeImg(this)"><div><h4>${e[0]}</h4><p>${e[2]} sets · ${e[3]} reps · ${e[1]}</p><p class="homeLoad">${e[0]==='Pull Up'?'Bodyweight':load+' lb'}${plate?' · '+plate.replace(/<[^>]+>/g,''):''}</p></div><span class="go">›</span></button>`;
  }).join('');
...
window.chooseApexSession=function(i){
  activeDay=i;localStorage.apexDay=i;updateHome();renderProgram();renderHomeFlow();toast(`${DAYS[i].name} selected`);
};
window.startApexExercise=function(name){
  singleExercise=name;singleOrigin='home';openWorkout();setActiveNav('Home');setTimeout(applySingleMode,0);
};
function applySingleMode(){
  if(!singleExercise)return;
  const list=document.getElementById('workoutList');if(!list)return;
  list.querySelectorAll('.lift').forEach(l=>l.style.display=l.dataset.name===singleExercise?'':'none');
  list.querySelectorAll('.groupHead').forEach(h=>h.style.display='none');
  let note=list.querySelector('.singleModeNote');if(!note){note=document.createElement('div');note.className='singleModeNote';list.prepend(note);}note.textContent=`Logging ${singleExercise}. Finish this exercise and you’ll return to Home.`;
  const finish=document.querySelector('#workout .finish');if(finish){finish.textContent='Done';finish.setAttribute('onclick','finishApexExercise()');}
}
window.finishApexExercise=function(){
  if(!singleExercise)return finishWorkout();
  const el=[...document.querySelectorAll('#workoutList .lift')].find(x=>x.dataset.name===singleExercise);if(!el)return;
  const minRep=Number(el.dataset.min),done=[...el.querySelectorAll('.setrow')].filter(r=>r.querySelector('.check').classList.contains('done'));
  if(!done.length){toast('Complete at least one set first');return;}
  const weight=Number(done[0].querySelector('.weight').value)||0,sets=done.map(r=>({reps:Number(r.querySelector('.reps').value)||0,minRep}));
  hist(singleExercise).push({date:new Date().toLocaleDateString('en-CA'),weight,sets,minRep});save();clearInterval(timerInt);
  const completedName=singleExercise;singleExercise=null;
  const finish=document.querySelector('#workout .finish');if(finish){finish.textContent='Finish';finish.setAttribute('onclick','finishWorkout()');}
  updateHome();show(singleOrigin==='program'?'program':'home');setActiveNav(singleOrigin==='program'?'Program':'Home');toast(`${completedName} saved`);
};
